...
public:
     ...
     */
    using iterator_category =   std::forward_iterator_tag;  //forward_iterator_tage = struct forward_iterator_tag : public input_iterator_tag {};是一个空类型
    using difference_type   =   std::ptrdiff_t;             //std::ptrdiff_t is the signed integer type of the result of subtracting two pointers.
     ...
     */
    operator HashMapIterator<Map, true>() const {   //operator type() const 类型转换运算符，可用于隐式转换
     ...
     */
    HashMapIterator<Map, IsConst>& operator++();        //prifix
    HashMapIterator<Map, IsConst> operator++(int);      //postfix, int不被使用，编译器会自动提供一个0，有个int仅仅是为了让编译器区分prefix和postfix。
    ...
    friend bool operator==(const HashMapIterator<Map_, IsConst_>& lhs, const HashMapIterator<Map_, IsConst_>& rhs); //目前不理解为什么这里是友元函数，成员函数不行么？
    /*
