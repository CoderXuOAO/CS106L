...
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>& rhs): 
    ...
    for(int i = 0; i < bucket_count(); ++i) {
        if(rhs._buckets_array[i] != nullptr) {
            _buckets_array[i] = new node(*rhs._buckets_array[i]);
            node *p1 = _buckets_array[i], *p2 = rhs._buckets_array[i]->next;
            while(p2 != nullptr) {
                p1->next = new node(*p2);
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(const HashMap<K, M, H>& rhs) {
    ...
    _size = rhs._size, _hash_function = rhs._hash_function;
    _buckets_array = rhs._buckets_array;
    for(int i = 0; i < bucket_count(); ++i) {
        if(rhs._buckets_array[i] != nullptr) {
            _buckets_array[i] = new node(*rhs._buckets_array[i]);
...
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>&& rhs) {
    ...
    _buckets_array = std::move(_buckets_array);
}
...
template <typename K, typename M, typename H>
const typename HashMap<K, M, H>::const_iterator HashMap<K, M, H>::end() const {
    return static_cast<const const_iterator>(const_cast<HashMap<K, M, H>*>(this)->end());
}
